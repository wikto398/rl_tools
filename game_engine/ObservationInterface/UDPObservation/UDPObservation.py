...
class UDPObservation(ObservationInterface):
    """Generic UDP observation interface.

    Receives a raw datagram and parses it as MessagePack. Games with a custom
    wire format (e.g. the flat binary protocol in
    ``torch_files/StrategyUDPObservation``) subclass this and override
    ``parse_observation``.
    """
    ...
    def parse_observation(self, raw_observation: bytes | None) -> dict | None:
        """Parse the raw observation data (MessagePack by default)."""
