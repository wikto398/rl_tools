...
class HeadlessGameEngineFactory:
    ...
    def create(self, game_engine_type: HeadlessGameEngine.GameEngineType | None = None, instance_id: int = 0, run_args: list| None = None, run_kwargs: dict | None = None, **kwargs):
        game_engine_type = game_engine_type if game_engine_type else HeadlessGameEngine.GameEngineType.GODOT
        run_args = run_args if run_args else []
        run_kwargs = run_kwargs if run_kwargs else {}
        kwargs = kwargs
        if game_engine_type in self.mapping:
            return self.mapping[game_engine_type](instance_id=instance_id, run_args=run_args, run_kwargs=run_kwargs, **kwargs)
        else:
